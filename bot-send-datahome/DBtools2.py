	...
	try:
		if(CheckTableEmpty(tablename) == True):
			...
			for line in data:
				...
				#cur.execute("""insert into kaidee (data) values (%s)""",[str(json.loads(line))])
		else:
		...
		conn.close()
		return data	
#def savedata_havestatus(data,tablename):
#	result = []
#	conn = psycopg2.connect(DATABASE_URL, sslmode='require')
#	cur = conn.cursor()
#	try:
...
#			for line in data:
#				result.append(json.loads(line))
#				query = sql.SQL("insert into {table} (data,status) values (%s,%s)").format(table=sql.Identifier(tablename))
#				cur.execute(query,[str(json.loads(line)),str("wait")])
#				#cur.execute("""insert into kaidee (data) values (%s)""",[str(json.loads(line))])
#		else:
#			print("Is Not Empty Table")
#			checksizetable(tablename)
...
#				for line in duplicateresult:
#					result.append(json.loads(line))
#					query = sql.SQL("insert into {table} (data,status) values (%s)").format(table=sql.Identifier(tablename))
#					cur.execute(query,[str(json.loads(line)),str("wait")])
#					#cur.execute("""insert into kaidee (data) values (%s)""",[str(json.loads(line))])
#		conn.commit()
#		cur.close()
...
#def getdata_havestatus(tablename):
#	print("Get Data Status")
#	conn = psycopg2.connect(DATABASE_URL, sslmode='require')
#	cur = conn.cursor()
#	query = sql.SQL("select * from {table} where status = 'wait' order by rowid asc").format(table=sql.Identifier(tablename))
#	cur.execute(query)
#	datatable = cur.fetchall()
#	dataintable = []
#	for i in datatable:
...
#def update_status(tablename,rowid):
#	conn = psycopg2.connect(DATABASE_URL, sslmode='require')
#	cur = conn.cursor()
#	try:
#		for i in rowid:
#			query = sql.SQL("UPDATE {table} SET status = 'done' WHERE rowid = (%s)").format(table=sql.Identifier(tablename))
#			cur.execute(query,[i])
